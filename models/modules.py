...
class UpSampleConvnext(nn.Module):
    def __init__(self, ratio, inchannel, outchannel,image=True):
        super().__init__()
        self.ratio = ratio
    ...
    def forward(self, x):
        if self.image:
            ...
            # 原处理：x = x.permute(0, 2, 1)
            # x = self.channel_reschedule(x)
            # x = x.permute(0,2, 1)
            # return self.upsample(x)
            # 文本处理
            return self.channel_reschedule(x)
...
class LayerNorm(nn.Module):
    ...
    image控制是否输入为图像，针对文本网络改变计算方式
    """
    def __init__(self, normalized_shape, eps=1e-6, data_format="channels_first", elementwise_affine = True, image=True):
        super().__init__()
    ...
    def forward(self, x):
        if self.image:
            ...
            if self.data_format == "channels_last":
                ...
                # 对于 3D 输入 (batch_size, channels, length)
                elif x.dim() == 3:
...
class ConvNextBlock(nn.Module):
    ...
    """
    def __init__(self, in_channel, hidden_dim, out_channel, kernel_size=3, layer_scale_init_value=1e-6, drop_path= 0.0,image=True):
        super().__init__()
        ...
        if self.image:
            ...
            #原处理： self.dwconv = nn.Conv1d(in_channel, in_channel, kernel_size=kernel_size, padding=(kernel_size - 1) // 2, groups=in_channel) # depthwise conv
            self.dwconv = nn.Linear(in_channel, in_channel)  # 对于文本，使用全连接层替代卷积
...
class TextNet(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.norm = LayerNorm(hidden_dim, image=False)
        self.act = nn.GELU()
